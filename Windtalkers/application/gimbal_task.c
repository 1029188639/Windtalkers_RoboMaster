...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    init->gimbal_yaw_motor.motor_gyro_set = init->gimbal_yaw_motor.motor_gyro;
		init->gimbal_yaw_motor.offset_ecd= 4959;
//		init->gimbal_yaw_motor.max_relative_angle= 0.35;
//		init->gimbal_yaw_motor.min_relative_angle= -0.35;


    init->gimbal_pitch_motor.absolute_angle_set = init->gimbal_pitch_motor.absolute_angle;
    init->gimbal_pitch_motor.relative_angle_set = init->gimbal_pitch_motor.relative_angle;
    init->gimbal_pitch_motor.motor_gyro_set = init->gimbal_pitch_motor.motor_gyro;
		init->gimbal_pitch_motor.offset_ecd= 4425;
		init->gimbal_pitch_motor.max_relative_angle= 0.35;
		init->gimbal_pitch_motor.min_relative_angle= -0.35;
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
		...
		//yaw轴没有限幅
		set_control->gimbal_yaw_motor.absolute_angle_set = rad_format(set_control->gimbal_yaw_motor.absolute_angle_set + add_yaw_angle);
	}
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    {
		//vision
		if(visionReceiveFlag == 1 && press_r == 1)
		{
			control_loop->gimbal_yaw_motor.absolute_angle_set =-(visionData.YawData/57.3f)+ control_loop->gimbal_yaw_motor.absolute_angle;	
		}																													
    ...
    else if (control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
		...
		if(visionReceiveFlag == 1 && press_r == 1)
		{
			control_loop->gimbal_pitch_motor.absolute_angle_set=(visionData.PitchData/57.3f)+control_loop->gimbal_pitch_motor.absolute_angle;  
			visionReceiveFlag = 0;
		}
