...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    chassis_move_init->vy_min_speed = -NORMAL_MAX_CHASSIS_SPEED_Y;
		...
		chassis_move_init->wz_max_speed = NORMAL_MAX_CHASSIS_SPEED_W;
		chassis_move_init->wz_min_speed = -NORMAL_MAX_CHASSIS_SPEED_W;
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set,chassis_move_t *chassis_move_rc_to_vector)
{
	...
	//不跟随云台模式，遥控底盘不能左右平移
	if(switch_is_mid(chassis_move_rc_to_vector->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
	{
		vy_set_channel = 0;
	}
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
    {
		...
		//G键开启小陀螺
		if(chassis_move_control->chassis_RC->key.v & CHASSIS_GRYO_ON_KEY)
		{
			GRYO_FLAG = 1;
		}
		else if(chassis_move_control->chassis_RC->key.v & CHASSIS_GRYO_OFF_KEY)
		{
			GRYO_FLAG = 0;
		}
		if(GRYO_FLAG == 0)
...
//			num = rand()%10;
			...
			if(addFlag)
				...
				ramp_calc(&cass, -1);
			chassis_move_control->wz_set = cass.out;
		}
    ...
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_NO_FOLLOW_YAW)
    {
        ...
        //“angle_set” 是旋转速度控制
        chassis_move_control->wz_set = -fp32_constrain(angle_set, chassis_move_control->wz_min_speed, chassis_move_control->wz_max_speed);
        chassis_move_control->vx_set = -fp32_constrain(vx_set, chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
        chassis_move_control->vy_set = -fp32_constrain(vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
    }
