...
void shoot_init(void)
{
	...
	static const fp32 Trigger_angle_pid[3] = {0.4, 0, 0};
	...
	PID_init(&shoot_control.trigger_Angle_pid, PID_POSITION, Trigger_angle_pid, TRIGGER_READY_PID_MAX_OUT, TRIGGER_READY_PID_MAX_IOUT);
    //更新数据
...
//		//角度环，速度环串级pid调试	
//		shoot_control.set_angle = PID_calc(&shoot_control.trigger_motor_pid, shoot_control.speed, shoot_control.speed_set);
//		shoot_control.given_current = (uint16_t)PID_calc(&shoot_control.trigger_Angle_pid, shoot_control.angle, shoot_control.set_angle);

		//角度环，速度环串级pid调试	
//		shoot_control.set_pid_angle = PID_calc(&shoot_control.trigger_Angle_pid, shoot_control.angle, shoot_control.set_angle);
//		shoot_control.given_current = (uint16_t)PID_calc(&shoot_control.trigger_motor_pid, shoot_control.speed, shoot_control.set_pid_angle);
//		
		//角度环，速度环串级pid调试
...
static void shoot_feedback_update(void)
{
    ...
    //SHIFT键按下加速摩擦轮，使得左键高速射击
    static uint16_t up_time = 0;
    if (shoot_control.shoot_rc->key.v & SHOOT_HIGH_KEYBOARD)
    {
        up_time = UP_ADD_TIME;
    }
